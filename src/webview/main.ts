...
let draft: RequestDraft | undefined;
let response: ResponseSnapshot | undefined;
let dirty = false;
let running = false;
let candidate: RequestDraft | undefined;
let environments: RequestEnvironment[] = [];
let activeEnvironmentId: string | undefined;
let history: HistoryEntry[] = [];
let savedRequests: RequestDraft[] = [];
let hydration: StudioHydration | undefined;
let requestTab = savedUi?.requestTab ?? 'params';
let responseTab = savedUi?.responseTab ?? 'body';
...
function required<T extends HTMLElement>(id: string): T {
  ...
  if (!node) { throw new Error(`Missing Request Studio element: ${id}`); }
  return node as T;
}
...
function buildShell(): void {
  ...
  const copyCurl = button('Copy cURL');
  copyCurl.id = 'copyCurl';
  const duplicate = button('Duplicate');
  duplicate.id = 'duplicateRequest';
  toolbar.append(method, url, run, cancel, copyCurl, duplicate);
  ...
  responseSection.append(
    ...
    tabBar('response', [
      ['body', 'Body'], ['headers', 'Headers'], ['raw', 'Raw'], ['error', 'Error'],
    ]),
  ...
  utilities.append(
    savedRequestsSection(), environmentSection(), historySection(), privacySection()
  );
...
function tabBar(group: string, tabs: Array<[string, string]>): HTMLElement {
...
function tabPanel(group: string, id: string): HTMLElement {
  const panel = element('div', { className: 'tab-panel' });
  panel.dataset.groupPanel = group;
  panel.dataset.panel = id;
  panel.setAttribute('role', 'tabpanel');
  return panel;
}

function selectTab(group: string, id: string): void {
  document.querySelectorAll<HTMLElement>(`[data-group="${group}"]`).forEach(tab => {
    const active = tab.dataset.tab === id;
    tab.classList.toggle('active', active);
    tab.setAttribute('aria-selected', String(active));
  });
  document.querySelectorAll<HTMLElement>(`[data-group-panel="${group}"]`).forEach(panel => {
    panel.classList.toggle('active', panel.dataset.panel === id);
  });
  if (group === 'request') { requestTab = id; } else { responseTab = id; }
  vscode.setState({ requestTab, responseTab });
}

function panel(group: string, id: string): HTMLElement {
  const node = document.querySelector<HTMLElement>(
    `[data-group-panel="${group}"][data-panel="${id}"]`
  );
  if (!node) { throw new Error(`Missing ${group}/${id} panel`); }
  return node;
}

function buildRequestPanels(): void {
  ...
  for (const [value, label] of [
    ['none', 'None'], ['json', 'JSON'], ['text', 'Raw text'], ['form', 'Form URL encoded'],
  ]) {
...
function buildResponsePanels(): void {
...
function environmentSection(): HTMLElement {
...
function savedRequestsSection(): HTMLElement {
  const section = element('section', { className: 'card utility-card' });
...
function bindStaticEvents(): void {
  ...
  required('candidateImport').addEventListener('click', () => {
    if (dirty && !window.confirm('Replace your edited request with the clipboard request?')) { return; }
  ...
  required('saveNamedRequest').addEventListener('click', () => {
    ...
    if (name) { vscode.postMessage({ type: 'saveNamedRequest', draft, name }); }
  });
  ...
  required('environmentSelect').addEventListener('change', event => {
    const environmentId = (event.target as HTMLSelectElement).value || undefined;
...
function loadDraft(next: RequestDraft, clearExistingResponse = true): void {
  if (clearExistingResponse) { clearResponse(); }
  draft = structuredClone(next);
  dirty = false;
  required('draftState').textContent = 'Parsed';
  (required<HTMLSelectElement>('method')).value = draft.method;
  (required<HTMLInputElement>('url')).value = draft.url;
  (required<HTMLSelectElement>('bodyMode')).value = draft.body.mode;
  (required<HTMLTextAreaElement>('bodyText')).value = draft.body.text;
...
function updateFromControls(): void {
  ...
  draft.method = (required<HTMLSelectElement>('method').value as RequestDraft['method']);
  draft.url = required<HTMLInputElement>('url').value;
  draft.body.mode = required<HTMLSelectElement>('bodyMode').value as RequestDraft['body']['mode'];
  draft.body.text = required<HTMLTextAreaElement>('bodyText').value;
  dirty = true;
...
function renderPairs(kind: 'query' | 'headers'): void {
...
function pairRow(kind: 'query' | 'headers', pair: EditablePair): HTMLElement {
  ...
  if (!pair.sensitive) { reveal.classList.add('invisible'); }
  ...
  const commit = () => {
    ...
    required('draftState').textContent = 'Modified';
    updateUrlPreview();
    if (draft) { vscode.postMessage({ type: 'updateDraft', draft }); }
    requestCurlPreview();
  };
...
function addPair(kind: 'query' | 'headers'): void {
  if (!draft) { draft = emptyDraft(); }
...
function updateUrlPreview(): void {
  ...
  if (!draft) { preview.textContent = ''; return; }
  try {
...
function validateBody(): boolean {
  ...
  const format = required<HTMLButtonElement>('formatJson');
...
function formatJson(): void {
  if (!draft || draft.body.mode !== 'json') { return; }
  try {
...
function copyCurl(): void {
  if (draft) { vscode.postMessage({ type: 'copyCurl', draft }); }
}

function duplicateRequest(): void {
  if (!draft) { return; }
...
let curlTimer: number | undefined;
function requestCurlPreview(): void {
  if (!draft) { return; }
  window.clearTimeout(curlTimer);
  curlTimer = window.setTimeout(() => {
    if (draft) { vscode.postMessage({ type: 'previewCurl', draft }); }
  }, 180);
}

function setRunning(next: boolean): void {
  running = next;
  required<HTMLButtonElement>('runRequest').disabled = next;
...
function clearResponse(): void {
  ...
  required<HTMLInputElement>('responseSearch').value = '';
}

function renderResponse(next: ResponseSnapshot): void {
  response = next;
  setRunning(false);
  ...
  summary.textContent = `${next.status} ${next.statusText || ''} · ${next.durationMs} ms · ${formatBytes(next.sizeBytes)} · ${next.redirectCount} redirect(s)${next.truncated ? ' · truncated' : ''} · ${new Date(next.receivedAt).toLocaleString()} · ${next.finalUrl}`;
  required('responseError').textContent = 'No network error.';
  renderResponseBody();
...
function renderResponseBody(): void {
  const target = required('responseBody');
  if (!response) { return; }
  let text = response.bodyText;
  if (response.contentType?.includes('json')) {
    try { text = JSON.stringify(JSON.parse(text), null, 2); } catch { /* raw fallback */ }
  }
  const query = required<HTMLInputElement>('responseSearch').value.toLowerCase();
  if (query) {
...
function renderResponseHeaders(): void {
...
function rawResponse(value: ResponseSnapshot): string {
  const headers = value.headers.map(header =>
    `${header.name}: ${header.sensitive ? '[masked]' : header.value}`
  ).join('\n');
...
function renderSavedRequests(): void {
  ...
  for (const item of savedRequests) {
    ...
    load.addEventListener('click', () => {
      if (dirty && !window.confirm('Replace your edited request with this saved request?')) { return; }
...
function renderEnvironments(): void {
  const select = required<HTMLSelectElement>('environmentSelect');
  ...
  required<HTMLButtonElement>('editEnvironment').disabled = !activeEnvironmentId;
}

function editEnvironment(environment: RequestEnvironment): void {
  ...
  const renderVariables = () => {
    ...
    for (const variable of environment.variables) {
      ...
      row.append(enabled, variableName, variableValue, labeledControl(secret, 'Secret'), remove);
  ...
  add.addEventListener('click', () => {
    environment.variables.push({
      id: newId(), name: '', value: '', enabled: true, secret: false,
    });
    renderVariables();
  });
...
function labeledControl(control: HTMLElement, text: string): HTMLElement {
  const label = element('label', { className: 'inline-label' });
  label.append(control, document.createTextNode(text));
  return label;
}

function setNotice(level: 'info' | 'warning' | 'error', message: string): void {
  const notice = required('notice');
  notice.textContent = message;
  notice.className = `notice ${level}`;
}

function hideCandidate(): void {
  candidate = undefined;
  required('candidateNotice').classList.add('hidden');
}

function showCandidate(next: RequestDraft, automaticEligible: boolean): void {
  candidate = next;
  const notice = required('candidateNotice');
  notice.classList.remove('hidden');
  const span = notice.querySelector('span');
  if (span) {
    span.textContent = automaticEligible
      ? 'An auto-run eligible request was detected in the clipboard.'
      : 'A new request was detected in the clipboard.';
  }
}

function applyHydration(payload: StudioHydration): void {
  hydration = payload;
  ...
  if (payload.response) { renderResponse(payload.response); }
}

function applyHostState(
  settings: StudioHydration['settings'],
  workspaceTrusted: boolean,
  executionLocation: string
): void {
  required('executionLocation').textContent = `Running from: ${executionLocation}`;
  required('trustState').textContent = workspaceTrusted
    ? 'Workspace trusted'
    : 'Restricted Mode — Run disabled';
  required<HTMLButtonElement>('runRequest').disabled = !workspaceTrusted;
...
window.addEventListener('message', event => {
  ...
  switch (message.type) {
    ...
    case 'draftImported':
      ...
      hideCandidate();
      if (message.warnings.length) { setNotice('warning', message.warnings.join(' ')); }
      break;
    ...
    case 'requestSucceeded':
      renderResponse(message.response as ResponseSnapshot);
      setNotice('info', 'Response received.');
      break;
    case 'requestFailed': {
      setRunning(false);
      const failure = message.failure as { category: string; message: string };
    ...
    case 'requestCancelled':
      ...
      break;
    case 'curlGenerated':
      required<HTMLTextAreaElement>('curlPreview').value = message.curl;
      break;
    case 'historyChanged':
